...
import { createAdaptiveTest } from '../lib/testforge-adaptive-db';
...
type View = 'dashboard' | 'lessons' | 'history' | 'analytics' | 'adaptive' | 'scan';
...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    loadWorkspace(supabase, user.id)
      ...
      .catch((error) => {
        if (mounted) setAppMessage(error instanceof Error ? error.message : 'Your workspace could not be loaded.');
      })
      .finally(() => {
  ...
  const adaptivePlan = useMemo(
    () => buildAdaptivePlan(classes, tests, attempts, activeClassId || null),
    [classes, tests, attempts, activeClassId]
  );
  ...
  async function startAdaptivePractice() {
    if (!user || !supabase || !currentClass) return;
    setAdaptiveBusy(true);
    setAppMessage('Building a personalized practice set from your question bank…');
    try {
      const selected = selectAdaptiveQuestions(
        tests,
        attempts,
        currentClass.id,
        adaptivePlan.recommendedQuestionCount
      );
      if (selected.length < 5) throw new Error('Add at least 5 reusable questions to this class before starting adaptive practice.');
      const title = `Adaptive Practice — ${currentClass.name}`;
      const next = await createAdaptiveTest(supabase, user.id, currentClass.id, title, selected);
      setTests((current) => [next, ...current]);
      setAppMessage('');
      startTest(next);
    } catch (error) {
      setAppMessage(error instanceof Error ? error.message : 'The adaptive practice set could not be created.');
    } finally {
  ...
  async function scanFile(file: File, existingLesson: LessonSource | null = null, modeOverride?: ScanMode) {
    ...
    setScanQuestions([]);
    setScanSource('');
    setScanFileBlob(file);
    setScanLessonSource(existingLesson);
    ...
    data.append('mode', effectiveMode);
  ...
  async function storeLessonOnly(file: File) {
    ...
    setAppMessage('Saving lesson file to your private library…');
    try {
      ...
      setAppMessage(error instanceof Error ? error.message : 'The lesson file could not be stored.');
    } finally {
  ...
  async function openLesson(lesson: LessonSource) {
    ...
    try {
      ...
      setAppMessage(error instanceof Error ? error.message : 'The source file could not be opened.');
    } finally {
      setLessonBusyId('');
    }
  }
  ...
  async function generateFromLesson(lesson: LessonSource) {
    if (!supabase) return;
    setLessonBusyId(lesson.id);
    setAppMessage('Preparing the saved lesson for a new assessment…');
    try {
      ...
      setAppMessage(error instanceof Error ? error.message : 'The saved lesson could not be used to generate a test.');
    } finally {
      setLessonBusyId('');
    }
  }
  ...
  function updateQuestion(index: number, patch: Partial<Question>) {
    setScanQuestions((current) => current.map((question, itemIndex) => itemIndex === index ? { ...question, ...patch } : question));
  }
  ...
  async function saveImportedTest() {
    ...
    try {
      ...
      if (scanLessonSource) {
        ...
        const saved = await createTestFromImport(
          supabase,
          user.id,
          activeClassId,
          scanTitle.trim() || 'Imported Test',
          scanQuestions,
          { file: scanFileBlob, sourceMode: scanSource || 'extracted' }
        );
        next = saved.test;
      ...
      setScanQuestions([]);
      setScanSource('');
      setScanFileBlob(null);
      setScanLessonSource(null);
    ...
    return (
      <main className="auth-shell">
        <section className="auth-card setup-card">
          ...
          <p className="muted">The schema supports private lessons, analytics-ready topics, and persistent adaptive practice tests.</p>
        </section>
      </main>
    );
  }
    ...
    return (
      <main className="auth-shell">
        <section className="auth-card">
          ...
          <p>{authMode === 'signup' ? 'Create a private workspace that keeps your classes, lessons, tests, grades, and personalized study plan synced across devices.' : 'Sign in to access your saved lessons, assessments, graded reviews, analytics, and adaptive practice.'}</p>
          <form className="auth-form" onSubmit={submitAuth}>
            {authMode === 'signup' && <label><span>Name</span><input value={authName} onChange={(event) => setAuthName(event.target.value)} placeholder="Your name" required /></label>}
          ...
          <button className="auth-switch" onClick={() => { setAuthMode(authMode === 'signin' ? 'signup' : 'signin'); setAuthMessage(''); }}>{authMode === 'signin' ? 'New to TestForge? Create an account' : 'Already have an account? Sign in'}</button>
        </section>
      </main>
    );
  }
  ...
  if (reviewAttempt) {
    ...
    return (
      <AttemptReview
        ...
        test={reviewTest}
        className={classes.find((item) => item.id === reviewAttempt.classId)?.name || 'Class'}
        onClose={() => { setReviewAttempt(null); setActiveView('history'); }}
        onRetake={(test) => startTest(test)}
      />
    );
  }
      ...
      <main className="exam-shell">
        ...
        {graded && (
          <section className="score-banner">
            ...
            <p>{currentScore} of {runningTest.questions.length} correct · saved to your history and study plan</p>
          </section>
        )}
    ...
    <main className="app-shell">
      <aside className="sidebar">
        ...
        <nav>
          ...
          <button className={activeView === 'adaptive' ? 'nav-item active' : 'nav-item'} onClick={() => setActiveView('adaptive')}><BrainCircuit size={18} /> Study Plan</button>
      ...
      <section className="main-panel">
        <header className="topbar">
          ...
          <div className="profile-grade"><div className="profile-copy"><strong>{profileName}</strong><span>Overall grade</span></div><div className="grade-badge">{analytics.overallGrade}</div></div>
        </header>
        {appMessage && <div className="app-message">{appMessage}</div>}
          ...
          <>
            <section className="hero-card">
              <div>
                ...
                <p>{currentClass ? `${classLessons.length} saved lesson${classLessons.length === 1 ? '' : 's'} · ${classTests.length} test${classTests.length === 1 ? '' : 's'} · everything synced to your private workspace.` : 'Create a class section first, then save lessons or generate tests inside it.'}</p>
              </div>
              {currentClass ? (
                <div className="hero-actions">
                  <button className="ghost-button" onClick={() => setActiveView('lessons')}><FileArchive size={18} /> Lesson library</button>
                  <button className="ghost-button" onClick={() => setActiveView('adaptive')}><BrainCircuit size={18} /> Study plan</button>
                  <button className="primary-button" onClick={() => setActiveView('scan')}><Sparkles size={18} /> Build a test</button>
                </div>
              ) : <button className="primary-button" onClick={addClass}><Plus size={18} /> Create class</button>}
            </section>

            {attempts.length > 0 && (
              <section className="content-section dashboard-analytics">
                <div className="section-header"><div><span className="eyebrow">Progress snapshot</span><h2>Performance</h2></div><button className="text-action" onClick={() => setActiveView('analytics')}>Open analytics <ChevronRight size={16} /></button></div>
                <AnalyticsKpis analytics={analytics} />
              </section>
            )}

            {currentClass && adaptivePlan.items.length > 0 && (
              <section className="content-section adaptive-snapshot">
                <div className="section-header"><div><span className="eyebrow">Personalized next step</span><h2>Study Next</h2></div><button className="text-action" onClick={() => setActiveView('adaptive')}>Open study plan <ChevronRight size={16} /></button></div>
                <div className="adaptive-snapshot-card">
                  <BrainCircuit size={22} />
                  <div><strong>{adaptivePlan.sessionFocus}</strong><span>{adaptivePlan.readiness === 'ready' ? `${adaptivePlan.recommendedQuestionCount}-question adaptive session ready` : 'Keep building the question bank to unlock adaptive practice'}</span></div>
                </div>
              </section>
            )}
                ...
                <div className="test-grid">
                  {classTests.map((test) => (
                    <article className="test-card" key={test.id}>
                      ...
                      <div><span className="muted">{test.questions.length} questions</span><h3>{test.title}</h3><p>{test.importId ? 'Linked to a saved lesson source.' : test.title.startsWith('Adaptive Practice') ? 'Personalized practice assembled from your question bank.' : 'Take this assessment in the TestForge testing environment.'}</p></div>
                      <button className="card-action" onClick={() => startTest(test)}>Start test <ChevronRight size={17} /></button>
                    </article>
                  ))}
                  {classTests.length === 0 && <div className="empty-state">No tests here yet. Open the lesson library or import a source file to create one.</div>}
                </div>
              </section>
            )}
          </>
        ...
        {activeView === 'lessons' && (
          <section className="content-section lesson-section">
            {!currentClass ? <div className="empty-state">Create a class before storing lesson files.</div> : (
              <>
                <div className="section-header"><div><span className="eyebrow">Reusable source library</span><h2>{currentClass.name} lessons</h2></div><span>{classLessons.length} saved</span></div>
                <label className={lessonUploadBusy ? 'lesson-upload busy' : 'lesson-upload'}>
                  <UploadCloud size={24} />
                  <div><strong>{lessonUploadBusy ? 'Saving lesson…' : 'Add lesson without generating a test'}</strong><span>Store PDF, DOCX, TXT, or Markdown now and build one or more assessments from it later.</span></div>
                  <input type="file" accept=".pdf,.docx,.txt,.md" disabled={lessonUploadBusy} onChange={(event) => event.target.files?.[0] && storeLessonOnly(event.target.files[0])} />
                </label>
                <div className="lesson-grid">
                  {classLessons.map((lesson) => {
                    const linkedTests = tests.filter((test) => test.importId === lesson.id).length;
                    const available = Boolean(lesson.storagePath);
                    const busy = lessonBusyId === lesson.id;
                    return (
                      <article className="lesson-card" key={lesson.id}>
                        <div className="lesson-card-top"><div className="lesson-file-icon"><FileArchive size={22} /></div><span className={available ? 'storage-pill' : 'storage-pill legacy'}>{available ? 'Private file' : 'Metadata only'}</span></div>
                        <div><span className="muted">{formatBytes(lesson.sizeBytes)} · {new Date(lesson.createdAt).toLocaleDateString()}</span><h3>{lesson.title}</h3><p>{lesson.fileName}</p><div className="lesson-stats"><span>{linkedTests} linked test{linkedTests === 1 ? '' : 's'}</span><span>{lesson.sourceMode}</span></div></div>
                        <div className="lesson-actions">
                          <button className="ghost-button" disabled={!available || busy} onClick={() => openLesson(lesson)}><ExternalLink size={16} /> Open source</button>
                          <button className="primary-button" disabled={!available || busy || scanning} onClick={() => generateFromLesson(lesson)}><Sparkles size={16} /> {busy ? 'Preparing…' : 'Generate new test'}</button>
                        </div>
                      </article>
                    );
                  })}
                  {classLessons.length === 0 && <div className="empty-state">No source files saved yet. Add a lesson here, or use Import / Generate and save the resulting test to preserve its source automatically.</div>}
                </div>
              </>
            )}
          </section>
        ...
        {activeView === 'history' && (
          <section className="content-section history-section">
            <div className="section-header"><div><span className="eyebrow">Review progress</span><h2>Completed & graded tests</h2></div><span>Tap any test to review every answer</span></div>
            <div className="history-list">
              {attempts.map((attempt) => (
                <button className="history-row history-row-button" key={attempt.id} onClick={() => setReviewAttempt(attempt)}>
                  <div><strong>{attempt.title}</strong><span>{classes.find((item) => item.id === attempt.classId)?.name || 'Class'} · {new Date(attempt.completedAt).toLocaleDateString()} · {attempt.score}/{attempt.total} correct</span></div>
                  <div className="history-score-group"><div className="history-score">{Math.round((attempt.score / attempt.total) * 100)}%</div><ChevronRight size={18} /></div>
                </button>
              ))}
        ...
        {activeView === 'adaptive' && (
          !currentClass
            ? <section className="content-section"><div className="empty-state">Create or select a class to build a personalized study plan.</div></section>
            : <AdaptiveStudyPlan plan={adaptivePlan} busy={adaptiveBusy} onStartAdaptive={startAdaptivePractice} />
        )}

        {activeView === 'scan' && (
          <section className="content-section scan-section">
            {!currentClass ? <div className="empty-state">Create a class before importing lesson material or building a test.</div> : (
              <>
                <div className="section-header">
                  <div><span className="eyebrow">Smart assessment builder</span><h2>{scanLessonSource ? `Build another test from ${scanLessonSource.title}` : 'Turn a lesson or test into an assessment'}</h2></div>
                  {scanLessonSource && <button className="ghost-button" onClick={() => { setScanLessonSource(null); setScanFileBlob(null); setScanQuestions([]); setScanMessage(''); setActiveView('lessons'); }}>Back to library</button>}
                </div>
                <div className="generation-controls">
                  <div className="generation-intro"><div className="generation-icon"><Sparkles size={20} /></div><div><strong>{scanLessonSource ? 'This test will stay linked to the saved lesson' : 'Choose how TestForge should read the file'}</strong><span>{scanLessonSource ? 'You can create multiple assessments from one source without uploading it again.' : 'Smart mode preserves existing tests and uses AI only when the upload is lesson material.'}</span></div></div>
                ...
                {scanning && scanLessonSource && <div className="reuse-progress"><Sparkles size={22} /><div><strong>Generating from your saved source…</strong><span>The original file stays private in your lesson library.</span></div></div>}
                {scanMessage && <p className="scan-message">{scanMessage}</p>}
                {scanQuestions.length > 0 && (
                  <div className="review-panel">
                    <div className="review-toolbar">
                      <div><div className="review-meta"><span className="eyebrow">Review before saving</span>{scanSource && <span className={scanSource === 'generated' ? 'source-pill generated' : 'source-pill'}>{scanSource === 'generated' ? 'AI generated' : 'Extracted from file'}</span>}</div><input className="title-input" value={scanTitle} onChange={(event) => setScanTitle(event.target.value)} /></div>
                      <button className="primary-button" disabled={savingImport} onClick={saveImportedTest}>{savingImport ? 'Saving…' : scanLessonSource ? 'Save linked test' : 'Save test + source'}</button>
                    </div>
                    {scanQuestions.map((question, questionIndex) => (
                      <article className="edit-question" key={question.id}>
                        ...
                        <label className="topic-editor"><span>Topic</span><input value={question.topic || 'General'} onChange={(event) => updateQuestion(questionIndex, { topic: event.target.value })} placeholder="e.g. Network Security" /></label>
