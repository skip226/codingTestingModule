...
type View = 'dashboard' | 'lessons' | 'history' | 'scan';
...
export default function HomePage() {
  ...
  async function signOut() {
    ...
    setRunningTest(null); setActiveView('dashboard');
  }
  ...
  function startTest(test: Test) {
    setRunningTest(test); setAnswers(Array(test.questions.length).fill(-1)); setGraded(false);
  }
  ...
  if (runningTest) {
    ...
    return (
      <main className="exam-shell">
        ...
        <section className="question-stack">{runningTest.questions.map((question, questionIndex) => <article className="question-card" key={question.id}><div className="question-number">Question {questionIndex + 1}</div><h2>{question.prompt}</h2><div className="answer-list">{question.options.map((option, optionIndex) => {
